...
df <- read.csv("C:/Users/imura/Documents/old_csv_downloads/Eva_Atta_colonies.csv", sep = ",", header = TRUE, stringsAsFactors = FALSE)
df$Bin = as.factor(df$Bin)
df$Colony = as.factor(df$Colony)
library(Rcpp)
library(lme4)
...
lda.trainff <- lda(Category ~ OL + AL + MB.MC+ MB.LC + MB.P + CX + SEZ, CV = TRUE, data = trainff)
#cannot incorporate ROCX bc "variables are collinear"

trainff$lda <- lda.trainff$class
table(trainff$lda,trainff$Category)

model <- lda(Category ~ OL + AL + MB.MC+ MB.LC + MB.P + CX + SEZ, data = trainff)
...
#graph points
predictions <- model %>% predict(testff)
lda.data <- CXind(trainff, predict(model)$x)
